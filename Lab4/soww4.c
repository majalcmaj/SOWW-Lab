...
int main(int argc, char **argv) { 
	...
	if(!thread_available) 
	{
		printf("Cannot start with threads - aborting");
		exit(1);
	}
	// find out my rank                                                                                                                                
	...
	if (proccount<1) {
		printf("Run with at least 1 process");
		MPI_Finalize();
		return -1;
	}
	...
	// shared(myrank, proccount, stdout, a, b) private(requests, i, resulttemp, sentcount, range, ranges, requestcompleted, result, recvcount, status) 
...
# pragma omp parallel sections num_threads(2) default(none) \
	 shared(myrank,a,b,proccount, ompi_mpi_comm_world, ompi_mpi_double, ompi_request_null, stdout) private(requests,ranges,resulttemp,sentcount,i,range,result,recvcount,requestcompleted,status)
	{
#pragma omp section
		{
			printf("Section: 1, myrank: %d, omp thread number: %d\n", myrank, omp_get_thread_num());
			fflush(stdout);
			if (myrank==0) {
				...
				for(i=0;i<proccount;i++) {
					...
					MPI_Isend(range,2,MPI_DOUBLE,i,DATA,MPI_COMM_WORLD,&(requests[2*proccount+i]));
				}
#ifdef DEBUG
...
#pragma omp section
		{
			printf("Section: 2, myrank: %d, omp thread number: %d\n", myrank, omp_get_thread_num());
			fflush(stdout);
