...
int main(int argc, char **argv) { 
	...
	if (myrank==0) {
		...
		for(i=0;i<proccount * 2;i++) {
			...
			MPI_Send(range,2,MPI_DOUBLE, i % proccount ,DATA,MPI_COMM_WORLD);
			sentcount++;
			range[0]=range[1];
		}
		do {
			// distribute remaining subranges to the processes which have completed their parts
			MPI_Iprobe(MPI_ANY_SOURCE, RESULT, MPI_COMM_WORLD, &flag, MPI_STATUS_IGNORE);
			if(flag != 0) {
				...
				printf("Master node computes range (%lf, %lf)", range[0], range[1]);
				range[1]=range[0]+RANGESIZE;
				if (range[1]>b) range[1]=b;
...
#ifdef DEBUGG
			printf("Master probed and the flag was %d\n", flag);
			fflush(stdout);
#endif
		} while (range[1]<b);
		// now receive results from the processes
		for(i=0;i<(proccount-1) * 2;i++) {
			MPI_Recv(&resulttemp,1,MPI_DOUBLE,MPI_ANY_SOURCE,RESULT,MPI_COMM_WORLD,&status);
#ifdef DEBUG
			printf("\nMaster received result %f from process %d",resulttemp,status.MPI_SOURCE);
			fflush(stdout);
